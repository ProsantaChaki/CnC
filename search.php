							...
							<div class="gallery-pagination-inner">
								<ul>
									...
									<?php									
										...
										$total_info =  $dbClass->getResultList("select p.product_id,p.name,p.code, pm.product_image, round(pr.rate,1) rate, p.category_id, c.name category_name, s.name size_name
																				from products p 
																				left join product_image pm on pm.product_id=p.product_id
																				left join product_rate pr on pr.product_id=p.product_id
																				left join category c on c.id=p.category_id
