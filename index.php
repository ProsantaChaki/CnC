					...
					<div class="menu-wrapper">
						<div class="portfolioFilter">
							<div class="portfolioFilter-inner">
								...
								<?php
								$cate_array = array();
								$category_info = $dbClass->getResultList("select * from category where status=1 order by id Desc");										
								...
								<?php
									foreach($cate_array as $key=>$category_id)
									{
										...
										c.id category_id, p.availability, c.name as cat_name, pim.product_image,
											r.discounted_rate as p_rate, tags , s.name as size
											FROM products p 
											LEFT JOIN product_rate r on r.product_id = p.product_id
											LEFT JOIN category c on c.id = p.category_id
											LEFT JOIN size s on s.id = r.size_id
											LEFT JOIN product_image pim on pim.product_id=p.product_id 
											where category_id=$category_id and p.availability=1 ORDER BY RAND() limit 0,2");							
											...
											echo "
												<div class='col-md-6 col-sm-6 col-xs-12 isotope-item $cat_name'>
													<div class='menu-list'>
														...
														<h5><span style='text-align:left'>$name ($size)</span> <span style='position: absolute;right:0'>".number_format($p_rate,2)."</span></h5>
														<p>$details</p>
													</div>
												</div>";	
										}
