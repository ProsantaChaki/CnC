...
if($q=="forget_password"){
	...
	$query="select email, username, customer_id from customer_infos WHERE  email='".$forget_email."'  and status=1";
		...
		if($stmt -> rowCount()>0){
			...
			$body = "Dear $username,  your new password is $new_password Please change the auto generated password right after login for security purpose.";
			
			$to 	 = $forget_email;
			$from 	 = 'admin@cakencookies.net';
			$subject = "Reset Cakencookie password";
