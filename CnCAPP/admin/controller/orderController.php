...
switch ($q){
	case "insert_or_update":
		if(isset($order_id) && $order_id == ""){
			...
			if(isset($coupon_name) && $coupon_name != ""){
				...
				$columns_value['coupon_id'] = $coupon[0];
			}
			...
			$columns_value = array(
				...
				'delivery_date'=>$delivery_date,
				'delivery_type'=>$delivery_option,
				'delivery_charge_id'=>$delivery_option_list,
				'delivery_charge' =>number_format($delivery_charge_amount,2),
				'address'=>$address,
				'remarks'=>$remarks, 
				'payment_status'=>$payment_status,
	...
	case "grid_data":
		...
		$countsql = "SELECT count(order_id)
					...
					WHERE CONCAT(order_id, customer_name, p_name, product_rate) LIKE '%$search_txt%' $condition"; 
		//echo $countsql;die;
	...
	case "get_order_details":
		...
		if($update_permission==1){
			$sql = "SELECT m.order_id, m.customer_id, 
					...
					ifnull(m.outlet_id,0) outlet_id, o.address, m.address, m.delivery_charge_id, m.tax_amount,  ifnull(cu.cupon_no,'') cupon_no, m.cupon_id,
					...
					case payment_method when payment_method=1 then 'bKash' when payment_method=2 then 'Rocket'  else 'Cash On Delevery'  end payment_method
					FROM order_master m
					LEFT JOIN order_details d ON d.order_id = m.order_id
					LEFT JOIN customer_infos c ON c.customer_id = m.customer_id
					LEFT JOIN outlets o ON o.id = m.outlet_id
					...
					LEFT JOIN cupons cu ON cu.id = m.cupon_id 
	...
	case "coupon_info":
		...
		if($count>0){
			foreach ($result as $row) {
				$json[] = array('id' => $row["c_type"],'label' => $row["c_info"],'rate' => $row["amount"]);
			}
		} else {
	...
	case "product_info":
		...
		if(isset($category_id) && $category_id !=0){
			$con = " WHERE p.category_id = $category_id " ;
		}else{
			$con = "";
		}
	...
	case "ad_product_info":
		$sql_query = "SELECT p.product_id, p.name FROM products p ORDER BY p.product_id";
		$stmt = $conn->prepare($sql_query);
		$stmt->execute();
		$json = array();
		$result = $stmt->fetchAll(PDO::FETCH_ASSOC);			
