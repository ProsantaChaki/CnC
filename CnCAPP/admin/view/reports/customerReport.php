...
$(document).ready(function () {	
	...
	load_customer_grid = function load_customer_grid(search_txt){
		...
		$.ajax({
			...
			success: function(data) {
				...
				if($.trim(search_txt) == "Print"){
					...
					html +='<button class="no-print" onclick="window.print()">Print</button><div width="100%"  style="text-align:center"><img src="'+employee_import_url+'/images/logo.png" width="80"/></div><h2 style="text-align:center">Cakencookie</h2><h4 style="text-align:center">Customer Report</h4><table width="100%"><tr><th width="60%" style="text-align:left"><small>'+serach_areas+'</small></th><th width="40%"  style="text-align:right"><small>Printed By: '+user_name+', Date:'+todate+'</small></th></tr></table>';
					
					if(!jQuery.isEmptyObject(data.records)){
				
						html +='<table width="100%" cellpadding="10" border="1px" style="margin-top:10px;border-collapse:collapse"><thead><tr><th style="text-align:left">Id</th><th style="text-align:left">Name</th><th style="text-align:left">Email</th><th style="text-align:left">Contact No</th><th style="text-align:left">Coupon No</th><th style="text-align:center">Status</th><th style="text-align:center">Total Order</th></tr></thead><tbody>';
						$.each(data.records, function(i,data){
							//alert(data)	
							html += "<tr>";		
							html +="<td style='text-align:left'>"+data.customer_id+"</td>";
							html +="<td style='text-align:left'>"+data.full_name+"</td>";	 
							html +="<td style='text-align:left'>"+data.email+"</td>";
							html +="<td style='text-align:left'>"+data.contact_no+"</td>";	
							html +="<td style='text-align:left'>"+data.coupon_no+"</td>";	
							html +="<td style='text-align:center'>"+data.status_text+"</td>";
							html +="<td style='text-align:center'>"+data.no_of_order+"</td>";								
							html += '</tr>'; 
						});
						html +="</tbody></table>"
					}
					...
					WinId = window.open("", "Customer Report","width=850,height=800,left=50,toolbar=no,menubar=YES,status=YES,resizable=YES,location=no,directories=no, scrollbars=YES"); 
					WinId.document.open();
