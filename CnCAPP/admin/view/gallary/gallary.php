			...
			success: function(data){ 
				...
				if(!jQuery.isEmptyObject(data.attachment)){
					...
					$.each(data.attachment, function(i,data){				
						if($.trim(data) != ""){
							attachement_html   += '<div class="col-md-2 text-center"><img src="'+project_url+"document/gallary_attachment/thumb/"+$.trim(data.attachment)+'" class="img-thumbnail"><button type="button" class="btn btn-danger btn-xs remove_img" onclick="delete_gallary_image('+data.img_id+','+master_id+')"><span class="glyphicon glyphicon-remove"></span></button></div>';
