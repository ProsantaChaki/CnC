			...
			$.ajax({
				...
				success: function(data){
					...
					if($.isNumeric(data)==true && data == 3){
						alert("Product Already Ordered. Only update those category which Product is not Ordered.")
					}
			...
			success: function(data){
				...
				if(!jQuery.isEmptyObject(data.attachment)){
					...
					$.each(data.attachment, function(i,data){				
						if($.trim(data) != ""){
							attachement_html   += '<div class="col-md-2 text-center" id="product_image_'+data.img_id+'"><img src="'+project_url+"images/product/thumb/"+$.trim(data.product_image)+'" class="img-thumbnail"><button type="button" class="btn btn-danger btn-xs remove_img" onclick="delete_gallary_image('+data.img_id+','+product_id+')"><span class="glyphicon glyphicon-remove"></span></button></div>';
