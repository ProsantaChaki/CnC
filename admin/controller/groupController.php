...
switch ($q){
	...
	/* case "get_permission_details":	
		$permission_details = $dbClass->getResultList("select group_concat(aa.id,'*', aa.activity_name ,'*',up.`status`) module_activity_ids_actions
									from 
									user_group_permission up
									left join web_actions aa on aa.id=up.action_id
									where up.group_id= $group_id and aa.status=0");
	...
	case "get_permission_details":	
		$permission_details = $dbClass->getResultList("SELECT aa.id, aa.activity_name,up.`status`, up.group_id, m.module_name
									...
									where up.group_id= $group_id and aa.status=0 order by module_name");
		$permission_string = "";
		foreach ($permission_details as $row) {
